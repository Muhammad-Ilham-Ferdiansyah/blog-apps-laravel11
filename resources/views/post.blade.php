<x-layout>
    ...
    {{-- <article class="py-8 max-w-screen-md">
        ...
        <div>
            By
            <a href="/authors/{{ $post->author->username }}" class="text-base text-purple-700 hover:underline">{{ $post->author->name }}</a>
            in
            <a href="/categories/{{ $post->category->slug }}" class="text-base text-purple-700 hover:underline">{{ $post->category->name }}</a>
                    ...
                    <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                        ...
                        <div>
                            ...
                            <a href="/posts?categories={{ $post->category->slug }}">
                                <span class="bg-{{ $post->category->color }}-100 text-primary-800 text-xs font-medium inline-flex items-center px-2.5 py-1 rounded dark:bg-primary-200 dark:text-primary-800">
                                    <svg class="mr-1 mb-0.5 w-3 h-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="{{ $post->category->icon_path }}"></path></svg>
                                    {{ $post->category->name }}
                                </span>
                            </a>
                        </div>
